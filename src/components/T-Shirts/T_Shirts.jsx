...
const T_Shirts = () => {
  ...
  const [t_shirts, sett_shirts] = useState([])
  useEffect(() => {
    const shirtItems = user.data.filter(card => card.category === "T_Shirt");
    sett_shirts(shirtItems);
}, [user.data]);
useEffect(() => {
...
const filtereditems = user.getFilteredItems(t_shirts)
  return (
    <>
    <div>
        <Navbar/>
